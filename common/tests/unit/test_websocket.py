...
from axvn_common.configuration import (
...
from axvn_common.websocket import (
...
from axvn_common.models import WebsocketApiResponse, WebsocketApiRateLimit
...
def mock_registry(monkeypatch):
    ...
    monkeypatch.setattr("axvn_common.websocket.global_stream_connections", mock)
    return mock
...
def mock_user_registry(monkeypatch):
    ...
    monkeypatch.setattr("axvn_common.websocket.global_user_stream_connections", mock)
    return mock
...
class TestWebSocketCommon:
    ...
    @pytest.mark.asyncio
    @patch(
        "axvn_common.websocket.aiohttp.ClientSession.ws_connect",
    ...
    @pytest.mark.asyncio
    @patch(
        "axvn_common.websocket.WebSocketCommon.receive_loop", new_callable=AsyncMock
    )
    @patch(
        "axvn_common.websocket.aiohttp.ClientSession.ws_connect",
        new_callable=AsyncMock,
...
class TestWebSocketAPIBase:
    ...
    async def test_send_signed_message_single_connection(self, ws_api, mock_connection):
        ...
        with patch.object(
            ...
            "axvn_common.utils.websocket_api_signature", return_value="signature"
        ) as mock_signature:
