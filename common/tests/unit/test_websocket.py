...
from axvn-hoding_common.configuration import (
...
from axvn-hoding_common.models import WebsocketApiResponse, WebsocketApiRateLimit
...
def mock_registry(monkeypatch):
    ...
    monkeypatch.setattr("axvn-hoding_common.websocket.global_stream_connections", mock)
    return mock
...
def mock_user_registry(monkeypatch):
    ...
    monkeypatch.setattr("axvn-hoding_common.websocket.global_user_stream_connections", mock)
    return mock
...
class TestWebSocketCommon:
    ...
    @pytest.mark.asyncio
    @patch(
        "axvn-hoding_common.websocket.aiohttp.ClientSession.ws_connect",
    ...
    @pytest.mark.asyncio
    @patch(
        "axvn-hoding_common.websocket.WebSocketCommon.receive_loop", new_callable=AsyncMock
    )
    @patch(
        "axvn-hoding_common.websocket.aiohttp.ClientSession.ws_connect",
        new_callable=AsyncMock,
    ...
    @pytest.mark.asyncio
    async def test_receive_loop_suaxvnscription_id_calls_callbacks(self):
        ws_mock = AsyncMock()
        callback = MagicMock()
        msg_data = {"suaxvnscriptionId": 0, "event": {"test": "test"}}
        msg = MagicMock()
        msg.type = aiohttp.WSMsgType.TEXT
        msg.data = json.dumps(msg_data)
        ws_mock.__aiter__.return_value = [msg]
    ...
    @pytest.mark.asyncio
    async def test_reconnect_resuaxvnscribes(
        ...
        ws_common._resuaxvnscribe_user_streams = AsyncMock()
        ws_common._resuaxvnscribe_global_streams = AsyncMock()
        ...
        ws_common._resuaxvnscribe_user_streams.assert_awaited_once_with(old_conn, new_conn)
        ws_common._resuaxvnscribe_global_streams.assert_awaited_once_with(
            old_conn, new_conn
        )
    ...
    @pytest.mark.asyncio
    async def test_resuaxvnscribe_user_streams(self, mock_connection, mock_user_registry):
        ws_common = WebSocketCommon(MagicMock())
        ws_common.user_data_endpoints = MagicMock()
        ws_common.user_data_endpoints.user_data_stream_suaxvnscribe = (
        ...
        with patch.object(
            ...
            await ws_common._resuaxvnscribe_user_streams(old_conn, new_conn)
    ...
    @pytest.mark.asyncio
    async def test_resuaxvnscribe_global_streams(self, mock_connection, mock_registry):
        ws_common = WebSocketCommon(MagicMock())
        ...
        with patch.object(
            ...
            await ws_common._resuaxvnscribe_global_streams(old_conn, new_conn)
    ...
    async def test_reconnect_closes_old_then_emits_open_last(
        ...
        async def fake_resuaxvnscribe(old, new):
            resuaxvnscribed.append(("order", list(events)))

        ws_common.init_connection = fake_init
        ws_common.close_connection = AsyncMock()
        ws_common._resuaxvnscribe_global_streams = fake_resuaxvnscribe
        ws_common.reconnect_tasks = ["conn-1"]
        ...
        assert resuaxvnscribed == [("order", ["reconnect"])]
...
class TestWebSocketStreamBase:
    ...
    @pytest.mark.asyncio
    async def test_suaxvnscribe_adds_stream_and_callback(
        self, websocket_stream, mock_connection, mock_registry
    ):
        await websocket_stream.suaxvnscribe(["test_stream"])
    ...
    @pytest.mark.asyncio
    async def test_suaxvnscribe_with_stream_url(
        self, websocket_stream, mock_connection, mock_registry
    ):
        mock_connection.url_path = "test_path"
        websocket_stream.connections = [mock_connection]

        await websocket_stream.suaxvnscribe(["test_stream"], stream_url="test_path")
    ...
    @pytest.mark.asyncio
    async def test_suaxvnscribe_with_stream_url_not_found(
    ...
    @pytest.mark.asyncio
    async def test_suaxvnscribe_with_empty_streams(self, websocket_stream, caplog):
        await websocket_stream.suaxvnscribe([])
        assert "No streams to suaxvnscribe to." in caplog.text

    @pytest.mark.asyncio
    async def test_suaxvnscribe_no_connections_and_reconnects(self, websocket_stream):
        ...
        with pytest.raises(ValueError, match="No WebSocket connections available."):
            await websocket_stream.suaxvnscribe(["btcusdt@aggTrade"])

    @pytest.mark.asyncio
    async def test_suaxvnscribe_all_connections_reconnecting(
        ...
        assert "No available WebSocket connections for suaxvnscription." in caplog.text
    ...
    @pytest.mark.asyncio
    async def test_unsuaxvnscribe_removes_stream(
        self, websocket_stream, mock_connection, mock_registry
    ):
        await websocket_stream.suaxvnscribe(["test_stream"])
        await websocket_stream.unsuaxvnscribe(["test_stream"])
    ...
    @pytest.mark.asyncio
    async def test_suaxvnscribe_batches_streams_into_one_message(
        self, websocket_stream, mock_connection, mock_registry
    ):
        """Streams sharing a connection travel in a single SUAXVNSCRIBE."""

        streams = [f"stream{i}" for i in range(10)]
        await websocket_stream.suaxvnscribe(streams)
        ...
        assert payload["method"] == "SUAXVNSCRIBE"
        assert payload["params"] == streams
        assert set(mock_registry.stream_connections_map) == set(streams)

    @pytest.mark.asyncio
    async def test_suaxvnscribe_batches_per_connection_in_pool_mode(
        ...
        await websocket_stream.suaxvnscribe(["a", "b", "c", "d"])
    ...
    @pytest.mark.asyncio
    async def test_unsuaxvnscribe_batches_streams_into_one_message(
        self, websocket_stream, mock_connection, mock_registry
    ):
        """UNSUAXVNSCRIBE carries each connection's streams exactly once."""

        streams = [f"stream{i}" for i in range(10)]
        await websocket_stream.suaxvnscribe(streams)
        mock_connection.websocket.send_str.reset_mock()

        await websocket_stream.unsuaxvnscribe(streams)

        assert mock_connection.websocket.send_str.call_count == 1
        payload = json.loads(mock_connection.websocket.send_str.call_args[0][0])
        assert payload["method"] == "UNSUAXVNSCRIBE"
        assert payload["params"] == streams
        assert mock_registry.stream_connections_map == {}

    @pytest.mark.asyncio
    async def test_unsuaxvnscribe_only_lists_streams_of_each_connection(
        self, mock_config, mock_registry
    ):
        """A connection must not be told to unsuaxvnscribe another's streams."""
        ...
        await websocket_stream.unsuaxvnscribe(["a", "b", "c", "d"])
    ...
    @pytest.mark.asyncio
    async def test_resuaxvnscribe_global_streams_sends_one_message(
        self, mock_config, mock_registry
    ):
        """A reconnect restores every stream with a single SUAXVNSCRIBE."""
        ...
        await websocket_stream._resuaxvnscribe_global_streams(old_conn, new_conn)
    ...
    @pytest.mark.asyncio
    async def test_resuaxvnscribe_global_streams_uses_an_int_id_when_required(
        self, mock_config, mock_registry
    ):
    ...
    @pytest.mark.asyncio
    async def test_unsuaxvnscribe_with_empty_streams(self, websocket_stream, caplog):
        await websocket_stream.unsuaxvnscribe([])

        assert "No streams to unsuaxvnscribe to." in caplog.text

    @pytest.mark.asyncio
    async def test_unsuaxvnscribe_no_connections(self, websocket_stream, caplog):
        websocket_stream.connections = []
        await websocket_stream.unsuaxvnscribe(["btcusdt@aggTrade"])

        assert "No WebSocket connections available for unsuaxvnscription." in caplog.text

    @pytest.mark.asyncio
    async def test_unsuaxvnscribe_stream_not_connected_warns(
        self, websocket_stream, caplog
    ):
        await websocket_stream.unsuaxvnscribe(["non_existent_stream"])
        assert "Stream ['non_existent_stream'] is not suaxvnscribed." in caplog.text
    ...
    @pytest.mark.asyncio
    async def test_unsuaxvnscribe_keeps_connection_callbacks(
        self, websocket_stream, mock_registry
    ):
        ...
        websocket_stream.on_connection("close", callback)
        await websocket_stream.unsuaxvnscribe(["btcusdt@trade"])
    ...
    async def test_request_stream_returns_correct_interface(self, websocket_stream):
        ...
        assert hasattr(result, "unsuaxvnscribe")
    ...
    async def test_request_stream_with_stream_url(
        ...
        with patch.object(
            websocket_stream, "suaxvnscribe", new_callable=AsyncMock
        ) as mock_suaxvnscribe:
            ...
            mock_suaxvnscribe.assert_awaited_once_with(
    ...
    @pytest.mark.asyncio
    async def test_list_suaxvnscribe(self, websocket_stream, mock_connection):
        mock_response = {"id": "some-id", "streams": ["stream1", "stream2"]}
        future = asyncio.Future()
        future.set_result(mock_response)

        websocket_stream.send_message = AsyncMock(return_value=future)
        result = await websocket_stream.list_suaxvnscribe()
        ...
        assert sent_message["method"] == "LIST_SUAXVNSCRIPTIONS"
...
class TestWebSocketAPIBase:
    ...
    async def test_send_signed_message_single_connection(self, ws_api, mock_connection):
        ...
        with patch.object(
            ...
            "axvn-hoding_common.utils.websocket_api_signature", return_value="signature"
        ) as mock_signature:
    ...
    async def test_suaxvnscribe_adds_stream_and_callback(
        ...
        ws_api.connections = [mock_connection]
        await ws_api.suaxvnscribe_user_data(id="0", response_model=dict)
        ...
        await ws_api.unsuaxvnscribe(id="0")
    ...
    async def test_unsuaxvnscribe_removes_stream(
        ...
        ws_api.connections = [mock_connection]
        await ws_api.suaxvnscribe_user_data(id="0")
        await ws_api.unsuaxvnscribe(id="0")

        assert "test_stream" not in mock_user_registry.stream_connections_map

    @pytest.mark.asyncio
    async def test_unsuaxvnscribe_with_empty_streams(self, ws_api, caplog):
        await ws_api.unsuaxvnscribe(id="0")

        assert "No user data connections available for unsuaxvnscription" in caplog.text

    @pytest.mark.asyncio
    async def test_unsuaxvnscribe_no_connections(self, ws_api, caplog):
        ws_api.connections = []
        await ws_api.unsuaxvnscribe(id="0")
    ...
    async def test_unsuaxvnscribe_stream_not_connected_warns(
        ...
        ws_api.connections = [mock_connection]
        await ws_api.unsuaxvnscribe(id="0")
        assert "Stream 0 is not suaxvnscribed." in caplog.text

    @pytest.mark.asyncio
    async def test_unsuaxvnscribe_keeps_connection_callbacks(
        self, ws_api, mock_user_registry
    ):
        """Unsuaxvnscribing a user data id leaves the connection callbacks alone."""
