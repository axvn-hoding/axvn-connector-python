...
from axvn_common.constants import TimeUnit
...
from axvn_common.models import RateLimit, WebsocketApiOptions
...
from axvn_common.headers import sanitize_header_value, parse_custom_headers
...
class TestWeb3Signature(unittest.TestCase):
    @patch("axvn_common.utils.clean_none_value", side_effect=lambda x: x)
    ...
    @patch("axvn_common.utils.get_signature", return_value="external_signature")
    @patch("axvn_common.utils.clean_none_value", side_effect=lambda x: x)
...
class TestSendRequest(unittest.TestCase):
    ...
    @patch("axvn_common.utils.encoded_string", side_effect=lambda x: x)
    @patch("axvn_common.utils.clean_none_value", side_effect=lambda x: x)
    @patch("axvn_common.utils.parse_rate_limit_headers", return_value=[])
    @patch("axvn_common.utils.get_timestamp", return_value=1234567890)
    @patch("axvn_common.utils.get_signature", return_value="signed_signature")
    ...
    @patch("axvn_common.utils.clean_none_value", side_effect=lambda x: x)
    @patch(
        "axvn_common.utils.should_retry_request",
    ...
    @patch("axvn_common.utils.encoded_string", side_effect=lambda x: x)
    @patch("axvn_common.utils.clean_none_value", side_effect=lambda x: x)
    @patch("axvn_common.utils.should_retry_request", return_value=False)
    ...
    @patch("axvn_common.utils.get_iso_8601", return_value="2026-06-03T10:20:30.123Z")
    @patch("axvn_common.utils.web3_signature", return_value="web3_signed_signature")
    @patch("axvn_common.utils.clean_none_value", side_effect=lambda x: x)
    @patch("axvn_common.utils.encoded_string", return_value="param=value")
    @patch("axvn_common.utils.parse_rate_limit_headers", return_value=[])
    ...
    @patch("axvn_common.utils.get_iso_8601", return_value="2026-06-03T10:20:30.123Z")
    @patch("axvn_common.utils.web3_signature")
    @patch("axvn_common.utils.clean_none_value", side_effect=lambda x: x)
    @patch("axvn_common.utils.encoded_string", return_value="param=value")
    @patch("axvn_common.utils.parse_rate_limit_headers", return_value=[])
...
class TestWsApiPayload(unittest.TestCase):
    ...
    def test_payload_with_api_key_and_signed(self):
        ...
        with patch(
            "axvn_common.utils.websocket_api_signature",
    ...
    def test_payload_generates_new_id_if_missing(self):
        ...
        with patch("axvn_common.utils.get_uuid", return_value="generated-id"):
            result = ws_api_payload(
...
class TestWebsocketApiSignature(unittest.TestCase):
    def test_websocket_api_signature(self):
        ...
        with patch(
            "axvn_common.utils.get_timestamp", return_value=1234567890
        ), patch(
            "axvn_common.utils.get_signature", return_value="mocked-signature"
