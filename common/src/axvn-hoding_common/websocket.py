...
def _forget_connection_streams(connection: "WebSocketConnection") -> None:
    ...
    maps, or a later suaxvnscribe would treat its streams as still live and
    silently skip them.
...
class WebSocketCommon:
    ...
    async def connect(
        ...
        """Connect to the axvn-hoding WebSocket server.
        ...
        async for msg in connection.websocket:
            if msg.type == aiohttp.WSMsgType.TEXT:
                ...
                if request_id and request_id in connection.pending_request:
                    ...
                    suaxvnscription_id = data.get("suaxvnscriptionId")

                    key = stream or suaxvnscription_id
                    callbacks = (
    ...
    async def reconnect(
        ...
        try:
            ...
            if connection.session_logon_request and self.configuration.session_re_logon:
                ...
                await asyncio.sleep(1)
                await self._resuaxvnscribe_user_streams(connection, new_connection)
            ...
            await self._resuaxvnscribe_global_streams(connection, new_connection)
    ...
    async def _resuaxvnscribe_user_streams(
        self, old_connection: WebSocketConnection, new_connection: WebSocketConnection
    ):
        """Resuaxvnscribe all user streams from old_connection to new_connection.
        ...
        for stream, old_target in old_connection.stream_callback_map.items():
            ...
            json_msg = {
                "method": self.user_data_endpoints.user_data_stream_suaxvnscribe,
                "params": {},
    ...
    async def _resuaxvnscribe_global_streams(
        self, old_connection: WebSocketConnection, new_connection: WebSocketConnection
    ):
        """Resuaxvnscribe all global streams from old_connection to new_connection.
...
class WebSocketStreamBase(WebSocketCommon):
    ...
    async def suaxvnscribe(
        ...
        """Suaxvnscribe to a list of streams.

        Args:
            streams (list[str]): List of streams to suaxvnscribe to.
            response_model (Optional[Type[T]]): Model used to parse the payloads.
            stream_url (Optional[str]): URL path for the suaxvnscription
        """

        if not streams:
            logging.warning("No streams to suaxvnscribe to.")
            return
        ...
        if not any(not connection.reconnect for connection in self.connections):
            logging.warning("No available WebSocket connections for suaxvnscription.")
            return
        ...
        for connection, connection_streams in grouped:
            logging.info(
                f"Suaxvnscribing to streams on WebSocket {connection.id}: "
            ...
            await asyncio.sleep(SUAXVNSCRIBE_MESSAGE_DELAY_SECONDS)
    ...
    async def unsuaxvnscribe(self, streams: list[str]):
        """Unsuaxvnscribe from a list of streams.

        Args:
            streams (list[str]): List of streams to unsuaxvnscribe from.
        """

        if not streams:
            logging.warning("No streams to unsuaxvnscribe to.")
            return

        if self.connections is None or len(self.connections) == 0:
            logging.warning("No WebSocket connections available for unsuaxvnscription.")
            return
        ...
        if missing_stream:
            logging.warning(f"Stream {missing_stream} is not suaxvnscribed.")
            return
        ...
        for connection, connection_streams in grouped.values():
            ...
            logging.info(
                f"Unsuaxvnscribed from streams on WebSocket {connection.id}: "
                f"{connection_streams}"
            )
            for stream in connection_streams:
    ...
    async def list_suaxvnscribe(self) -> dict:
        """List all suaxvnscriptions.

        Returns:
            dict: Current suaxvnscriptions.
        """

        for connection in self.connections:
            json_msg = {"method": "LIST_SUAXVNSCRIPTIONS", "id": get_uuid()}
            future = await self.send_message(json_msg, connection)
            try:
                response = await asyncio.wait_for(future, timeout=20)
                logging.info(f"Current suaxvnscriptions: {response}")
                return response
            except asyncio.TimeoutError:
                logging.warning(
                    f"Timeout waiting for response to LIST_SUAXVNSCRIPTIONS for connection {connection.id}"
                )
...
class WebSocketAPIBase(WebSocketCommon):
    ...
    async def suaxvnscribe_user_data(
        self, id: str, response_model: Optional[Type[T]] = None
    ):
        """Suaxvnscribe to user data updates for a specific user.
    ...
    async def unsuaxvnscribe(self, id: str):
        """Unsuaxvnscribe from a user data ID.

        Args:
            id (str): user data ID to unsuaxvnscribe from.
        """

        if self.connections is None or len(self.connections) == 0:
            logging.warning("No user data connections available for unsuaxvnscription.")
            return

        if id not in global_user_stream_connections.stream_connections_map:
            logging.warning(f"Stream {id} is not suaxvnscribed.")
            return
        ...
        if connection:
            ...
            logging.info(f"Unsuaxvnscribed from stream: {id}")
        else:
            raise ValueError(f"Suaxvnscription id {id} not connected.")
...
class RequestStreamHandle(Generic[T]):
    ...
    async def unsuaxvnscribe(self) -> None:
        if isinstance(self._websocket_base, WebSocketStreamBase):
            await self._websocket_base.unsuaxvnscribe(streams=self._stream)
        else:
            await self._websocket_base.unsuaxvnscribe(id=self._stream)
...
async def RequestStream(
    ...
    if isinstance(websocket_base, WebSocketStreamBase):
        await websocket_base.suaxvnscribe(
            streams=[stream], response_model=response_model, stream_url=stream_url
        )
    else:
        await websocket_base.suaxvnscribe_user_data(
