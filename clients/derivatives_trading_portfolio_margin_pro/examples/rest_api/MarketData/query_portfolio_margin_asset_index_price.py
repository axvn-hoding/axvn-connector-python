...
def query_portfolio_margin_asset_index_price():
    try:
        response = client.rest_api.query_portfolio_margin_asset_index_price()

        rate_limits = response.rate_limits
        logging.info(
            f"query_portfolio_margin_asset_index_price() rate limits: {rate_limits}"
        )

        data = response.data()
        logging.info(f"query_portfolio_margin_asset_index_price() response: {data}")
    except Exception as e:
        logging.error(f"query_portfolio_margin_asset_index_price() error: {e}")
