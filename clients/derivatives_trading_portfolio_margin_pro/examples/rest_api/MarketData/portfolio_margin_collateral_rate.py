...
def portfolio_margin_collateral_rate():
    try:
        response = client.rest_api.portfolio_margin_collateral_rate()

        rate_limits = response.rate_limits
        logging.info(f"portfolio_margin_collateral_rate() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"portfolio_margin_collateral_rate() response: {data}")
    except Exception as e:
        logging.error(f"portfolio_margin_collateral_rate() error: {e}")
