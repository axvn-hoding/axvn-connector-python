...
def repay_futures_negative_balance():
    try:
        response = client.rest_api.repay_futures_negative_balance()

        rate_limits = response.rate_limits
        logging.info(f"repay_futures_negative_balance() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"repay_futures_negative_balance() response: {data}")
    except Exception as e:
        logging.error(f"repay_futures_negative_balance() error: {e}")
