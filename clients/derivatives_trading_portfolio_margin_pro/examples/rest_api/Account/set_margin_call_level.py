...
def set_margin_call_level():
    try:
        ...
        logging.info(f"set_margin_call_level() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"set_margin_call_level() response: {data}")
    except Exception as e:
        logging.error(f"set_margin_call_level() error: {e}")


if __name__ == "__main__":
    set_margin_call_level()
