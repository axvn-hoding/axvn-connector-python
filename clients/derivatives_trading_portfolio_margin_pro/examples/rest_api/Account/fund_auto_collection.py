...
def fund_auto_collection():
    try:
        response = client.rest_api.fund_auto_collection()

        rate_limits = response.rate_limits
        logging.info(f"fund_auto_collection() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"fund_auto_collection() response: {data}")
    except Exception as e:
        logging.error(f"fund_auto_collection() error: {e}")
