...
def fund_collection_by_asset():
    try:
        ...
        logging.info(f"fund_collection_by_asset() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"fund_collection_by_asset() response: {data}")
    except Exception as e:
        logging.error(f"fund_collection_by_asset() error: {e}")
