...
def get_auto_repay_futures_status():
    try:
        response = client.rest_api.get_auto_repay_futures_status()

        rate_limits = response.rate_limits
        logging.info(f"get_auto_repay_futures_status() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_auto_repay_futures_status() response: {data}")
    except Exception as e:
        logging.error(f"get_auto_repay_futures_status() error: {e}")
