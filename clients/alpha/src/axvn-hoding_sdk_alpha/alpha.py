...
from axvn-hoding_common.constants import ALPHA_REST_API_PROD_URL, ALPHA_WS_STREAMS_PROD_URL
from . import metadata
from .rest_api import AlphaRestAPI
from .websocket_streams import AlphaWebSocketStreams
...
class Alpha:
    """Alpha API that exposes REST, and Stream APIs in a single interface."""
    ...
    @property
    def websocket_streams(self) -> AlphaWebSocketStreams:
        if self._websocket_streams is None and self._websocket_streams_config:
            ...
            self._websocket_streams = AlphaWebSocketStreams(
                self._websocket_streams_config
            )
