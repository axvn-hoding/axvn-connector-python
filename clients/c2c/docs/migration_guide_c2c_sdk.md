# Migration Guide: axvn-hoding C2C SDK Modularization

With the transition to a modularized structure, the axvn-hoding Connector has been split into separate Python libraries, each focusing on a distinct product (e.g., Spot, Futures, etc.). This guide explains how to migrate from the monolithic `axvn-hoding-connector` package to the new `axvn-hoding-sdk-c2c` library.
...
3. **Imports**:  
   ...
   **New:**
   ...
   ```python
   from axvn-hoding_sdk_c2c.c2c import C2C, ConfigurationRestAPI
   ```
