...
def query_sub_account_list():
    try:
        ...
        logging.info(f"query_sub_account_list() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"query_sub_account_list() response: {data}")
    except Exception as e:
        logging.error(f"query_sub_account_list() error: {e}")
