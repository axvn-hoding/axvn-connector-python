...
def enable_options_for_sub_account():
    try:
        ...
        logging.info(f"enable_options_for_sub_account() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"enable_options_for_sub_account() response: {data}")
    except Exception as e:
        logging.error(f"enable_options_for_sub_account() error: {e}")
