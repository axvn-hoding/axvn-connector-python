...
def delete_ip_list_for_a_sub_account_api_key():
    try:
        response = client.rest_api.delete_ip_list_for_a_sub_account_api_key(
            email="123@test.com",
        ...
        logging.info(
            f"delete_ip_list_for_a_sub_account_api_key() rate limits: {rate_limits}"
        )

        data = response.data()
        logging.info(f"delete_ip_list_for_a_sub_account_api_key() response: {data}")
    except Exception as e:
        logging.error(f"delete_ip_list_for_a_sub_account_api_key() error: {e}")
