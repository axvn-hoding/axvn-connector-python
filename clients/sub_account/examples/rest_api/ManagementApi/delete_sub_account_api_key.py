...
def delete_sub_account_api_key():
    try:
        response = client.rest_api.delete_sub_account_api_key(
            email="123@test.com",
            sub_account_api_key="k5V49ldtn4tszj6W3hystegdfvmGbqDzjmkCtpTvC0G74WhK7yd4rfCTo4lShf",
        )

        rate_limits = response.rate_limits
        logging.info(f"delete_sub_account_api_key() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"delete_sub_account_api_key() response: {data}")
    except Exception as e:
        logging.error(f"delete_sub_account_api_key() error: {e}")
