...
def modify_sub_account_api_key_permission():
    try:
        response = client.rest_api.modify_sub_account_api_key_permission(
            email="123@test.com",
        ...
        logging.info(
            f"modify_sub_account_api_key_permission() rate limits: {rate_limits}"
        )

        data = response.data()
        logging.info(f"modify_sub_account_api_key_permission() response: {data}")
    except Exception as e:
        logging.error(f"modify_sub_account_api_key_permission() error: {e}")
