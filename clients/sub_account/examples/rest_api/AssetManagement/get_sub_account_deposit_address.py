...
def get_sub_account_deposit_address():
    try:
        ...
        logging.info(f"get_sub_account_deposit_address() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_sub_account_deposit_address() response: {data}")
    except Exception as e:
        logging.error(f"get_sub_account_deposit_address() error: {e}")
