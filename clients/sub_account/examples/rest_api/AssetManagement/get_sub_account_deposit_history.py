...
def get_sub_account_deposit_history():
    try:
        ...
        logging.info(f"get_sub_account_deposit_history() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_sub_account_deposit_history() response: {data}")
    except Exception as e:
        logging.error(f"get_sub_account_deposit_history() error: {e}")
