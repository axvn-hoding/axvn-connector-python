...
def transfer_to_sub_account_of_same_master():
    try:
        ...
        logging.info(
            f"transfer_to_sub_account_of_same_master() rate limits: {rate_limits}"
        )

        data = response.data()
        logging.info(f"transfer_to_sub_account_of_same_master() response: {data}")
    except Exception as e:
        logging.error(f"transfer_to_sub_account_of_same_master() error: {e}")
