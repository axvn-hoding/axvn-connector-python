...
def query_sub_account_assets():
    try:
        ...
        logging.info(f"query_sub_account_assets() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"query_sub_account_assets() response: {data}")
    except Exception as e:
        logging.error(f"query_sub_account_assets() error: {e}")
