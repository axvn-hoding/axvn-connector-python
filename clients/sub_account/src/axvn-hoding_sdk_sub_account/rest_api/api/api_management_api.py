"""
Sub Account REST API

Create and manage sub-accounts, control permissions, and transfer assets via the Sub Account API.
The version of the OpenAPI document: 1.0.0
Generated by OpenAPI Generator (https://openapi-generator.tech)

Do not edit the class manually.
"""

from typing import Optional, Union
from requests import Session
from axvn-hoding_common.configuration import ConfigurationRestAPI
from axvn-hoding_common.errors import RequiredError
from axvn-hoding_common.models import ApiResponse
from axvn-hoding_common.signature import Signers
from axvn-hoding_common.utils import send_request

from ..models import AddIpRestrictionForSubAccountApiKeyResponse
from ..models import CreateSubAccountApiKeyResponse
from ..models import DeleteIpListForASubAccountApiKeyResponse

from ..models import GetIpRestrictionForASubAccountApiKeyResponse
from ..models import ModifySubAccountApiKeyPermissionResponse
from ..models import QuerySubAccountApiKeyResponse


class ApiManagementApi:
    """API Client for ApiManagementApi endpoints."""

    def __init__(
        self,
        configuration: ConfigurationRestAPI = None,
        session: Session = None,
        signer: Signers = None,
    ) -> None:
        self._configuration = configuration
        self._session = session
        self._signer = signer

    def add_ip_restriction_for_sub_account_api_key(
        self,
        email: Union[str, None],
        sub_account_api_key: Union[str, None],
        status: Union[int, None],
        ip_address: Optional[str] = None,
        recv_window: Optional[int] = None,
    ) -> ApiResponse[AddIpRestrictionForSubAccountApiKeyResponse]:
        """
                Add IP Restriction for Sub-Account API key (For Master Account) (USER_DATA)
                POST /sapi/v2/sub-account/subAccountApi/ipRestriction
                https://developers.axvn.vn/en/docs/catalog/vip-and-institutional-sub-account/api/rest-api/api-management#add-ip-restriction-for-sub-account-api-key

                Add IP Restriction for Sub-Account API key

        Weight(UID): 3000

        Security Type: USER_DATA

        Notes:
        - You need to enable Enable Spot & Margin Trading option for the api key which requests this endpoint

                Args:
                    email (Union[str, None]):
                    sub_account_api_key (Union[str, None]):
                    status (Union[int, None]): IP Restriction status. 1 = IP Unrestricted. 2 = Restrict access to trusted IPs only.
                    ip_address (Optional[str] = None): Insert static IP in batch, separated by commas.
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[AddIpRestrictionForSubAccountApiKeyResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if email is None:
            raise RequiredError(
                field="email", error_message="Missing required parameter 'email'"
            )
        if sub_account_api_key is None:
            raise RequiredError(
                field="sub_account_api_key",
                error_message="Missing required parameter 'sub_account_api_key'",
            )
        if status is None:
            raise RequiredError(
                field="status", error_message="Missing required parameter 'status'"
            )

        body = {}
        payload = {
            "email": email,
            "sub_account_api_key": sub_account_api_key,
            "status": status,
            "ip_address": ip_address,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="POST",
            path="/sapi/v2/sub-account/subAccountApi/ipRestriction",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=AddIpRestrictionForSubAccountApiKeyResponse,
            is_signed=True,
            signer=self._signer,
        )

    def create_sub_account_api_key(
        self,
        email: Union[str, None],
        api_name: Union[str, None],
        status: Union[int, None],
        can_trade: Optional[bool] = None,
        can_margin_loan_repay: Optional[bool] = None,
        can_futures_trade: Optional[bool] = None,
        can_universal_transfer: Optional[bool] = None,
        can_vanilla_options: Optional[bool] = None,
        ip_address: Optional[str] = None,
        third_party_name: Optional[str] = None,
        public_key: Optional[str] = None,
        recv_window: Optional[int] = None,
    ) -> ApiResponse[CreateSubAccountApiKeyResponse]:
        """
                Create Sub-account API Key (For Master Account) (USER_DATA)
                POST /sapi/v1/sub-account/subAccountApi
                https://developers.axvn.vn/en/docs/catalog/vip-and-institutional-sub-account/api/rest-api/api-management#create-sub-account-api-key

                Create a new API Key for a sub-account.

        Weight(UID): 3000

        Security Type: USER_DATA

        Notes:
        - `status=2` requires `ipAddress`
        - `status=3` requires `thirdPartyName`
        - Asset Sub Account is not supported
        - The caller must pass the KYC IP restriction check

                Args:
                    email (Union[str, None]): Sub-account email
                    api_name (Union[str, None]): API Key name
                    status (Union[int, None]): IP restriction status. 1 = unrestricted, 2 = restricted to trusted IPs, 3 = third-party IP restriction
                    can_trade (Optional[bool] = None): Spot & Margin trading permission, default false
                    can_margin_loan_repay (Optional[bool] = None): Margin borrow/repay permission, default false
                    can_futures_trade (Optional[bool] = None): Futures trading permission, default false
                    can_universal_transfer (Optional[bool] = None): Universal transfer permission, default false
                    can_vanilla_options (Optional[bool] = None): Vanilla options permission, default false
                    ip_address (Optional[str] = None): Required when status=2. IP address list, max 500 chars
                    third_party_name (Optional[str] = None): Required when status=3. Third-party name
                    public_key (Optional[str] = None): Ed25519 public key (optional, for Ed25519 type API Key)
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[CreateSubAccountApiKeyResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if email is None:
            raise RequiredError(
                field="email", error_message="Missing required parameter 'email'"
            )
        if api_name is None:
            raise RequiredError(
                field="api_name", error_message="Missing required parameter 'api_name'"
            )
        if status is None:
            raise RequiredError(
                field="status", error_message="Missing required parameter 'status'"
            )

        body = {}
        payload = {
            "email": email,
            "api_name": api_name,
            "status": status,
            "can_trade": can_trade,
            "can_margin_loan_repay": can_margin_loan_repay,
            "can_futures_trade": can_futures_trade,
            "can_universal_transfer": can_universal_transfer,
            "can_vanilla_options": can_vanilla_options,
            "ip_address": ip_address,
            "third_party_name": third_party_name,
            "public_key": public_key,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="POST",
            path="/sapi/v1/sub-account/subAccountApi",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=CreateSubAccountApiKeyResponse,
            is_signed=True,
            signer=self._signer,
        )

    def delete_ip_list_for_a_sub_account_api_key(
        self,
        email: Union[str, None],
        sub_account_api_key: Union[str, None],
        ip_address: Union[str, None],
        recv_window: Optional[int] = None,
    ) -> ApiResponse[DeleteIpListForASubAccountApiKeyResponse]:
        """
                Delete IP List For a Sub-account API Key (For Master Account) (USER_DATA)
                DELETE /sapi/v1/sub-account/subAccountApi/ipRestriction/ipList
                https://developers.axvn.vn/en/docs/catalog/vip-and-institutional-sub-account/api/rest-api/api-management#delete-ip-list-for-asub-account-api-key

                Delete IP List For a Sub-account API Key

        Weight(UID): 3000

        Security Type: USER_DATA

        Notes:
        - You need to enable Enable Spot & Margin Trading option for the api key which requests this endpoint

                Args:
                    email (Union[str, None]):
                    sub_account_api_key (Union[str, None]):
                    ip_address (Union[str, None]): IPs to be deleted. Can be added in batches, separated by commas
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[DeleteIpListForASubAccountApiKeyResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if email is None:
            raise RequiredError(
                field="email", error_message="Missing required parameter 'email'"
            )
        if sub_account_api_key is None:
            raise RequiredError(
                field="sub_account_api_key",
                error_message="Missing required parameter 'sub_account_api_key'",
            )
        if ip_address is None:
            raise RequiredError(
                field="ip_address",
                error_message="Missing required parameter 'ip_address'",
            )

        body = {}
        payload = {
            "email": email,
            "sub_account_api_key": sub_account_api_key,
            "ip_address": ip_address,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="DELETE",
            path="/sapi/v1/sub-account/subAccountApi/ipRestriction/ipList",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=DeleteIpListForASubAccountApiKeyResponse,
            is_signed=True,
            signer=self._signer,
        )

    def delete_sub_account_api_key(
        self,
        email: Union[str, None],
        sub_account_api_key: Union[str, None],
        recv_window: Optional[int] = None,
    ) -> ApiResponse[object]:
        """
                Delete Sub-account API Key (For Master Account) (USER_DATA)
                DELETE /sapi/v1/sub-account/subAccountApi
                https://developers.axvn.vn/en/docs/catalog/vip-and-institutional-sub-account/api/rest-api/api-management#delete-sub-account-api-key

                Delete an API Key of a sub-account.

        Weight(UID): 3000

        Security Type: USER_DATA

        Notes:
        - Asset Sub Account is not supported
        - The caller must pass the KYC IP restriction check

                Args:
                    email (Union[str, None]): Sub-account email
                    sub_account_api_key (Union[str, None]): The sub-account API Key to be deleted
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[object]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if email is None:
            raise RequiredError(
                field="email", error_message="Missing required parameter 'email'"
            )
        if sub_account_api_key is None:
            raise RequiredError(
                field="sub_account_api_key",
                error_message="Missing required parameter 'sub_account_api_key'",
            )

        body = {}
        payload = {
            "email": email,
            "sub_account_api_key": sub_account_api_key,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="DELETE",
            path="/sapi/v1/sub-account/subAccountApi",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=object,
            is_signed=True,
            signer=self._signer,
        )

    def get_ip_restriction_for_a_sub_account_api_key(
        self,
        email: Union[str, None],
        sub_account_api_key: Union[str, None],
        recv_window: Optional[int] = None,
    ) -> ApiResponse[GetIpRestrictionForASubAccountApiKeyResponse]:
        """
                Get IP Restriction for a Sub-account API Key (For Master Account) (USER_DATA)
                GET /sapi/v1/sub-account/subAccountApi/ipRestriction
                https://developers.axvn.vn/en/docs/catalog/vip-and-institutional-sub-account/api/rest-api/api-management#get-ip-restriction-for-asub-account-api-key

                Get IP Restriction for a Sub-account API Key

        Weight(UID): 3000

        Security Type: USER_DATA

                Args:
                    email (Union[str, None]):
                    sub_account_api_key (Union[str, None]):
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[GetIpRestrictionForASubAccountApiKeyResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if email is None:
            raise RequiredError(
                field="email", error_message="Missing required parameter 'email'"
            )
        if sub_account_api_key is None:
            raise RequiredError(
                field="sub_account_api_key",
                error_message="Missing required parameter 'sub_account_api_key'",
            )

        body = {}
        payload = {
            "email": email,
            "sub_account_api_key": sub_account_api_key,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="GET",
            path="/sapi/v1/sub-account/subAccountApi/ipRestriction",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=GetIpRestrictionForASubAccountApiKeyResponse,
            is_signed=True,
            signer=self._signer,
        )

    def modify_sub_account_api_key_permission(
        self,
        email: Union[str, None],
        sub_account_api_key: Union[str, None],
        can_trade: Optional[bool] = None,
        can_margin_loan_repay: Optional[bool] = None,
        can_futures_trade: Optional[bool] = None,
        can_universal_transfer: Optional[bool] = None,
        can_vanilla_options: Optional[bool] = None,
        recv_window: Optional[int] = None,
    ) -> ApiResponse[ModifySubAccountApiKeyPermissionResponse]:
        """
                Modify Sub-account API Key Permission (For Master Account) (USER_DATA)
                POST /sapi/v1/sub-account/subAccountApiPermission
                https://developers.axvn.vn/en/docs/catalog/vip-and-institutional-sub-account/api/rest-api/api-management#modify-sub-account-api-key-permission

                Modify the trading permissions of a sub-account API Key.

        Weight(UID): 3000

        Security Type: USER_DATA

        Notes:
        - Portfolio Margin Retail User is not supported
        - Asset Sub Account is not supported
        - The caller must pass the KYC IP restriction check

                Args:
                    email (Union[str, None]): Sub-account email
                    sub_account_api_key (Union[str, None]): Sub-account API Key
                    can_trade (Optional[bool] = None): Spot & Margin trading permission
                    can_margin_loan_repay (Optional[bool] = None): Margin borrow/repay permission
                    can_futures_trade (Optional[bool] = None): Futures trading permission
                    can_universal_transfer (Optional[bool] = None): Universal transfer permission
                    can_vanilla_options (Optional[bool] = None): Vanilla options permission
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[ModifySubAccountApiKeyPermissionResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if email is None:
            raise RequiredError(
                field="email", error_message="Missing required parameter 'email'"
            )
        if sub_account_api_key is None:
            raise RequiredError(
                field="sub_account_api_key",
                error_message="Missing required parameter 'sub_account_api_key'",
            )

        body = {}
        payload = {
            "email": email,
            "sub_account_api_key": sub_account_api_key,
            "can_trade": can_trade,
            "can_margin_loan_repay": can_margin_loan_repay,
            "can_futures_trade": can_futures_trade,
            "can_universal_transfer": can_universal_transfer,
            "can_vanilla_options": can_vanilla_options,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="POST",
            path="/sapi/v1/sub-account/subAccountApiPermission",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=ModifySubAccountApiKeyPermissionResponse,
            is_signed=True,
            signer=self._signer,
        )

    def query_sub_account_api_key(
        self,
        email: Union[str, None],
        sub_account_api_key: Optional[str] = None,
        page: Optional[int] = None,
        size: Optional[int] = None,
        recv_window: Optional[int] = None,
    ) -> ApiResponse[QuerySubAccountApiKeyResponse]:
        """
                Query Sub-account API Key (For Master Account) (USER_DATA)
                GET /sapi/v1/sub-account/subAccountApi
                https://developers.axvn.vn/en/docs/catalog/vip-and-institutional-sub-account/api/rest-api/api-management#query-sub-account-api-key

                Query the API Key list of a sub-account.

        Weight(UID): 3000

        Security Type: USER_DATA

                Args:
                    email (Union[str, None]): Sub-account email
                    sub_account_api_key (Optional[str] = None): Specify an API Key for exact match
                    page (Optional[int] = None): Page number, default 1, minimum 1
                    size (Optional[int] = None): Page size, default 30, maximum 100
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[QuerySubAccountApiKeyResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if email is None:
            raise RequiredError(
                field="email", error_message="Missing required parameter 'email'"
            )

        body = {}
        payload = {
            "email": email,
            "sub_account_api_key": sub_account_api_key,
            "page": page,
            "size": size,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="GET",
            path="/sapi/v1/sub-account/subAccountApi",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=QuerySubAccountApiKeyResponse,
            is_signed=True,
            signer=self._signer,
        )
