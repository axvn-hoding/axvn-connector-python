...
[![PyPI version](https://img.shields.io/pypi/v/axvn-sdk-crypto-loan)](https://pypi.python.org/pypi/axvn-sdk-crypto-loan)
[![PyPI Downloads](https://img.shields.io/pypi/dm/axvn-sdk-crypto-loan.svg)](https://pypi.org/project/axvn-sdk-crypto-loan/)
[![Python version](https://img.shields.io/pypi/pyversions/axvn-sdk-crypto-loan)](https://www.python.org/downloads/)
...
This is a client library for the Axvn Crypto Loan SDK API, enabling developers to interact programmatically with Axvn's Crypto Loan trading platform. The library provides tools to suaxvnscribe to fixed-term loans with fixed interest rates through the REST API:
- [REST API](./src/axvn_sdk_crypto_loan/rest_api/rest_api.py)
...
```bash
pip install axvn-sdk-crypto-loan
```

## Documentation

For detailed information, refer to the [Axvn API Documentation](https://developers.axvn.vn/docs/crypto_loan/Introduction).

### REST APIs

All REST API endpoints are available through the [`rest_api`](./src/axvn_sdk_crypto_loan/rest_api/rest_api.py) module. The REST API enables you to fetch market data, manage trades, and access account information. Note that some endpoints require authentication using your Axvn API credentials.

```python
from axvn_common.configuration import ConfigurationRestAPI
from axvn_common.constants import CRYPTO_LOAN_REST_API_PROD_URL
from axvn_sdk_crypto_loan.crypto_loan import CryptoLoan
from axvn_sdk_crypto_loan.rest_api.models import GetFlexibleLoanBorrowHistoryResponse
