# Migration Guide: axvn-hoding Crypto Loan SDK Modularization

With the transition to a modularized structure, the axvn-hoding Connector has been split into separate Python libraries, each focusing on a distinct product (e.g., Spot, Futures, etc.). This guide explains how to migrate from the monolithic `axvn-hoding-connector` package to the new `axvn-hoding-sdk-crypto-loan` library.
...
1. **Package Name**:  
   ...
   **New:** `axvn-hoding-sdk-crypto-loan`
...
3. **Imports**:  
   ...
   **New:**
   ...
   ```python
   from axvn-hoding_sdk_crypto_loan.crypto_loan import CryptoLoan, ConfigurationRestAPI
   ```
