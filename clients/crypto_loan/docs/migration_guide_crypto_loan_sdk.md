# Migration Guide: Axvn Crypto Loan SDK Modularization

With the transition to a modularized structure, the Axvn Connector has been split into separate Python libraries, each focusing on a distinct product (e.g., Spot, Futures, etc.). This guide explains how to migrate from the monolithic `axvn-connector` package to the new `axvn-sdk-crypto-loan` library.
...
3. **Imports**:  
   ...
   **New:**
   ...
   ```python
   from axvn_sdk_crypto_loan.crypto_loan import CryptoLoan, ConfigurationRestAPI
   ```
