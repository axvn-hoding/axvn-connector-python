...
def check_collateral_repay_rate():
    try:
        ...
        logging.info(f"check_collateral_repay_rate() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"check_collateral_repay_rate() response: {data}")
    except Exception as e:
        logging.error(f"check_collateral_repay_rate() error: {e}")
