# Migration Guide: axvn-hoding Pay SDK Modularization

With the transition to a modularized structure, the axvn-hoding Connector has been split into separate Python libraries, each focusing on a distinct product (e.g., Spot, Futures, etc.). This guide explains how to migrate from the monolithic `axvn-hoding-connector` package to the new `axvn-hoding-sdk-pay` library.
...
3. **Imports**:  
   ...
   **New:**
   ...
   ```python
   from axvn-hoding_sdk_pay.pay import Pay, ConfigurationRestAPI
   ```
