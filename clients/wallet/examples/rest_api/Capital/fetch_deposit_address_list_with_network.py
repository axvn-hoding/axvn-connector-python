...
def fetch_deposit_address_list_with_network():
    try:
        ...
        logging.info(
            f"fetch_deposit_address_list_with_network() rate limits: {rate_limits}"
        )

        data = response.data()
        logging.info(f"fetch_deposit_address_list_with_network() response: {data}")
    except Exception as e:
        logging.error(f"fetch_deposit_address_list_with_network() error: {e}")
