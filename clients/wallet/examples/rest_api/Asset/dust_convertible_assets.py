...
def dust_convertible_assets():
    try:
        ...
        logging.info(f"dust_convertible_assets() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"dust_convertible_assets() response: {data}")
    except Exception as e:
        logging.error(f"dust_convertible_assets() error: {e}")
