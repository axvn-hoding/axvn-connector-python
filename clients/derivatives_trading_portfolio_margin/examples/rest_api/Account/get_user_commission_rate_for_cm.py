...
def get_user_commission_rate_for_cm():
    try:
        ...
        logging.info(f"get_user_commission_rate_for_cm() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_user_commission_rate_for_cm() response: {data}")
    except Exception as e:
        logging.error(f"get_user_commission_rate_for_cm() error: {e}")
