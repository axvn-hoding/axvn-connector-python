...
def new_um_conditional_order():
    try:
        ...
        logging.info(f"new_um_conditional_order() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"new_um_conditional_order() response: {data}")
    except Exception as e:
        logging.error(f"new_um_conditional_order() error: {e}")
