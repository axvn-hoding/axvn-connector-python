...
def new_cm_conditional_order():
    try:
        response = client.rest_api.new_cm_conditional_order(
            symbol="BTCUSDT",
        ...
        logging.info(f"new_cm_conditional_order() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"new_cm_conditional_order() response: {data}")
    except Exception as e:
        logging.error(f"new_cm_conditional_order() error: {e}")
