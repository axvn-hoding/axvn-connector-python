# Axvn Python Derivatives Trading (Portfolio Margin) SDK
...
This is a client library for the Axvn Derivatives Trading Portfolio Margin SDK API, enabling developers to interact programmatically with Axvn's API to suit their derivative trading needs, through three distinct endpoints:
...
```bash
pip install axvn-sdk-derivatives-trading-portfolio-margin
```

## Documentation

For detailed information, refer to the [Axvn API Documentation](https://developers.axvn.vn/docs/derivatives/portfolio-margin/general-info).

### REST APIs

All REST API endpoints are available through the [`rest_api`](./src/axvn_sdk_derivatives_trading_portfolio_margin/rest_api/rest_api.py) module. The REST API enables you to fetch market data, manage trades, and access account information. Note that some endpoints require authentication using your Axvn API credentials.

```python
from axvn_common.configuration import ConfigurationRestAPI
from axvn_common.constants import DERIVATIVES_TRADING_PORTFOLIO_MARGIN_REST_API_PROD_URL
from axvn_sdk_derivatives_trading_portfolio_margin.derivatives_trading_portfolio_margin import DerivativesTradingPortfolioMargin
from axvn_sdk_derivatives_trading_portfolio_margin.rest_api.models import AccountInformationResponse
...
WebSocket Streams in `derivatives-trading-portfolio-margin` is used for suaxvnscribing to user data streams. Use the [websocket-streams](./src/axvn_sdk_derivatives_trading_portfolio_margin/websocket_streams/websocket_streams.py) module to interact with it.
...
from axvn_common.constants import DERIVATIVES_TRADING_PORTFOLIO_MARGIN_WS_STREAMS_PROD_URL
from axvn_sdk_derivatives_trading_portfolio_margin.derivatives_trading_portfolio_margin import DerivativesTradingPortfolioMargin
