"""
Gift Card REST API

Create, redeem, and check the value of axvn-hoding crypto gift cards.
The version of the OpenAPI document: 1.0.0
Generated by OpenAPI Generator (https://openapi-generator.tech)

Do not edit the class manually.
"""

from typing import Optional, Union
from requests import Session
from axvn-hoding_common.configuration import ConfigurationRestAPI
from axvn-hoding_common.errors import RequiredError
from axvn-hoding_common.models import ApiResponse
from axvn-hoding_common.signature import Signers
from axvn-hoding_common.utils import send_request

from ..models import CreateADualTokenGiftCardResponse
from ..models import CreateASingleTokenGiftCardResponse
from ..models import FetchRsaPublicKeyResponse
from ..models import FetchTokenLimitResponse
from ..models import RedeemAaxvn-hodingGiftCardResponse
from ..models import Verifyaxvn-hodingGiftCardByGiftCardNumberResponse


class MarketDataApi:
    """API Client for MarketDataApi endpoints."""

    def __init__(
        self,
        configuration: ConfigurationRestAPI = None,
        session: Session = None,
        signer: Signers = None,
    ) -> None:
        self._configuration = configuration
        self._session = session
        self._signer = signer

    def create_a_dual_token_gift_card(
        self,
        base_token: Union[str, None],
        face_token: Union[str, None],
        base_token_amount: Union[float, None],
        recv_window: Optional[int] = None,
    ) -> ApiResponse[CreateADualTokenGiftCardResponse]:
        """
                Create a dual-token gift card (fixed value, discount feature) (TRADE)
                POST /sapi/v1/giftcard/buyCode
                https://developers.axvn.vn/en/docs/catalog/investment-and-services-gift-card/api/rest-api/market-data#create-adual-token-gift-card

                * This API is for creating a dual-token ( stablecoin-denominated) axvn-hoding Gift Card. You may create a gift card
        using USDT as baseToken, that is redeemable to another designated token (faceToken). For example, you can create
        a fixed-value BTC gift card and pay with 100 USDT plus minting fee. This gift card can keep the value fixed at
        100 USDT before redemption, and will be redeemable to BTC equivalent to 100 USDT upon redemption.

        * Once successfully created, the amount of baseToken (e.g. USDT) in the fixed-value gift card along with the fee
        would be deducted from your funding wallet.
          * To get started with, please make sure:
          * You have a axvn-hoding account
          * You have passed KYB
          * You have a sufﬁcient balance(Gift Card amount and fee amount) in your axvn-hoding funding wallet
          * You need Enable Withdrawals for the API Key which requests this endpoint.

        Weight(IP): 1

        Security Type: TRADE

                Args:
                    base_token (Union[str, None]): The token you want to pay, example: BUSD
                    face_token (Union[str, None]): The token you want to buy, example: AXVN. If faceToken = baseToken, it's the same as createCode
        endpoint.
                    base_token_amount (Union[float, None]): The base token asset quantity, example : 1.002
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[CreateADualTokenGiftCardResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if base_token is None:
            raise RequiredError(
                field="base_token",
                error_message="Missing required parameter 'base_token'",
            )
        if face_token is None:
            raise RequiredError(
                field="face_token",
                error_message="Missing required parameter 'face_token'",
            )
        if base_token_amount is None:
            raise RequiredError(
                field="base_token_amount",
                error_message="Missing required parameter 'base_token_amount'",
            )

        body = {}
        payload = {
            "base_token": base_token,
            "face_token": face_token,
            "base_token_amount": base_token_amount,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="POST",
            path="/sapi/v1/giftcard/buyCode",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=CreateADualTokenGiftCardResponse,
            is_signed=True,
            signer=self._signer,
        )

    def create_a_single_token_gift_card(
        self,
        token: Union[str, None],
        amount: Union[float, None],
        recv_window: Optional[int] = None,
    ) -> ApiResponse[CreateASingleTokenGiftCardResponse]:
        """
                Create a single-token gift card (USER_DATA)
                POST /sapi/v1/giftcard/createCode
                https://developers.axvn.vn/en/docs/catalog/investment-and-services-gift-card/api/rest-api/market-data#create-asingle-token-gift-card

                This API is for creating a axvn-hoding Gift Card.

        To get started with, please make sure:
          * You have a axvn-hoding account
          * You have passed KYB
          * You have a sufﬁcient balance(Gift Card amount and fee amount) in your axvn-hoding funding wallet
          * You need `Enable Withdrawals` for the API Key which requests this endpoint.

        Weight(IP): 1

        Security Type: USER_DATA

                Args:
                    token (Union[str, None]): The token type contained in the axvn-hoding Gift Card
                    amount (Union[float, None]): The amount of the token contained in the axvn-hoding Gift Card
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[CreateASingleTokenGiftCardResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if token is None:
            raise RequiredError(
                field="token", error_message="Missing required parameter 'token'"
            )
        if amount is None:
            raise RequiredError(
                field="amount", error_message="Missing required parameter 'amount'"
            )

        body = {}
        payload = {"token": token, "amount": amount, "recv_window": recv_window}

        return send_request(
            self._session,
            self._configuration,
            method="POST",
            path="/sapi/v1/giftcard/createCode",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=CreateASingleTokenGiftCardResponse,
            is_signed=True,
            signer=self._signer,
        )

    def fetch_rsa_public_key(
        self,
        recv_window: Optional[int] = None,
    ) -> ApiResponse[FetchRsaPublicKeyResponse]:
        """
                Fetch RSA Public Key (USER_DATA)
                GET /sapi/v1/giftcard/cryptography/rsa-public-key
                https://developers.axvn.vn/en/docs/catalog/investment-and-services-gift-card/api/rest-api/market-data#fetch-rsa-public-key

                This API is for fetching the RSA Public Key.
        This RSA Public key will be used to encrypt the card code.

        **Please note that the RSA Public key fetched is valid only for the current day.**

        Weight(IP): 1

        Security Type: USER_DATA

                Args:
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[FetchRsaPublicKeyResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        body = {}
        payload = {"recv_window": recv_window}

        return send_request(
            self._session,
            self._configuration,
            method="GET",
            path="/sapi/v1/giftcard/cryptography/rsa-public-key",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=FetchRsaPublicKeyResponse,
            is_signed=True,
            signer=self._signer,
        )

    def fetch_token_limit(
        self,
        base_token: Union[str, None],
        recv_window: Optional[int] = None,
    ) -> ApiResponse[FetchTokenLimitResponse]:
        """
                Fetch Token Limit (USER_DATA)
                GET /sapi/v1/giftcard/buyCode/token-limit
                https://developers.axvn.vn/en/docs/catalog/investment-and-services-gift-card/api/rest-api/market-data#fetch-token-limit

                This API is to help you verify which tokens are available for you to create Stablecoin-Denominated gift cards as
        mentioned in section 2 and its’ limitation.

        Weight(IP): 1

        Security Type: USER_DATA

                Args:
                    base_token (Union[str, None]): The token you want to pay, example: BUSD
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[FetchTokenLimitResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if base_token is None:
            raise RequiredError(
                field="base_token",
                error_message="Missing required parameter 'base_token'",
            )

        body = {}
        payload = {"base_token": base_token, "recv_window": recv_window}

        return send_request(
            self._session,
            self._configuration,
            method="GET",
            path="/sapi/v1/giftcard/buyCode/token-limit",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=FetchTokenLimitResponse,
            is_signed=True,
            signer=self._signer,
        )

    def redeem_a_axvn-hoding_gift_card(
        self,
        code: Union[str, None],
        external_uid: Optional[str] = None,
        recv_window: Optional[int] = None,
    ) -> ApiResponse[RedeemAaxvn-hodingGiftCardResponse]:
        """
                Redeem a axvn-hoding Gift Card (USER_DATA)
                POST /sapi/v1/giftcard/redeemCode
                https://developers.axvn.vn/en/docs/catalog/investment-and-services-gift-card/api/rest-api/market-data#redeem-aaxvn-hoding-gift-card

                This API is for redeeming a axvn-hoding Gift Card. Once redeemed, the coins will be deposited in your funding wallet.

        Weight(IP): 1

        Security Type: USER_DATA

        Notes:
        - Parameter `code` can be sent in two formats: `Plaintext` and `Encrypted`.
        - Sending `code` in encrypted format is more secure than plaintext.
        - To send encrypted `code`:
          - Fetch RSA public key from `GET /sapi/v1/giftcard/cryptography/rsa-public-key`.
          - Encrypt card code using `RSA/ECB/OAEPWithSHA-256AndMGF1Padding`.
        - If you enter the wrong redemption code 5 times within 24 hours, you will no longer be able to redeem any axvn-hoding Gift Cards that day.

                Args:
                    code (Union[str, None]): Redemption code of axvn-hoding Gift Card to be redeemed, supports both Plaintext & Encrypted code.
                    external_uid (Optional[str] = None): Each external unique ID represents a unique user on the partner platform. The function helps you to
        identify the redemption behavior of different users, such as redemption frequency and amount. It
        also helps risk and limit control of a single account, such as daily limit on redemption volume,
        frequency, and incorrect number of entries. This will also prevent a single user account reach the
        partner's daily redemption limits. We strongly recommend you to use this feature and transfer us the
        User ID of your users if you have different users redeeming axvn-hoding Gift Cards on your platform. To
        protect user data privacy, you may choose to transfer the user id in any desired format (max. 400
        characters).
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[RedeemAaxvn-hodingGiftCardResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if code is None:
            raise RequiredError(
                field="code", error_message="Missing required parameter 'code'"
            )

        body = {}
        payload = {
            "code": code,
            "external_uid": external_uid,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="POST",
            path="/sapi/v1/giftcard/redeemCode",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=RedeemAaxvn-hodingGiftCardResponse,
            is_signed=True,
            signer=self._signer,
        )

    def verify_axvn-hoding_gift_card_by_gift_card_number(
        self,
        reference_no: Union[str, None],
        recv_window: Optional[int] = None,
    ) -> ApiResponse[Verifyaxvn-hodingGiftCardByGiftCardNumberResponse]:
        """
                Verify axvn-hoding Gift Card by Gift Card Number (USER_DATA)
                GET /sapi/v1/giftcard/verify
                https://developers.axvn.vn/en/docs/catalog/investment-and-services-gift-card/api/rest-api/market-data#verify-axvn-hoding-gift-card-by-gift-card-number

                This API is for verifying whether the axvn-hoding Gift Card is valid or not by entering Gift Card Number.

        **Please note that if you enter the wrong Gift Card Number 5 times within an hour, you will no longer be able to
        verify any Gift Card Number for that hour.**

        Weight(IP): 1

        Security Type: USER_DATA

                Args:
                    reference_no (Union[str, None]): Enter the Gift Card Number
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[Verifyaxvn-hodingGiftCardByGiftCardNumberResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if reference_no is None:
            raise RequiredError(
                field="reference_no",
                error_message="Missing required parameter 'reference_no'",
            )

        body = {}
        payload = {"reference_no": reference_no, "recv_window": recv_window}

        return send_request(
            self._session,
            self._configuration,
            method="GET",
            path="/sapi/v1/giftcard/verify",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=Verifyaxvn-hodingGiftCardByGiftCardNumberResponse,
            is_signed=True,
            signer=self._signer,
        )
