# Migration Guide: Axvn Gift Card SDK Modularization

With the transition to a modularized structure, the Axvn Connector has been split into separate Python libraries, each focusing on a distinct product (e.g., Spot, Futures, etc.). This guide explains how to migrate from the monolithic `axvn-connector` package to the new `axvn-sdk-gift-card` library.
...
3. **Imports**:  
   ...
   **New:**
   ...
   ```python
   from axvn_sdk_gift_card.gift_card import GiftCard, ConfigurationRestAPI
   ```
