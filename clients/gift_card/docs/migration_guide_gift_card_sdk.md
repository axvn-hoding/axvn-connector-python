# Migration Guide: axvn-hoding Gift Card SDK Modularization

With the transition to a modularized structure, the axvn-hoding Connector has been split into separate Python libraries, each focusing on a distinct product (e.g., Spot, Futures, etc.). This guide explains how to migrate from the monolithic `axvn-hoding-connector` package to the new `axvn-hoding-sdk-gift-card` library.
...
3. **Imports**:  
   ...
   **New:**
   ...
   ```python
   from axvn-hoding_sdk_gift_card.gift_card import GiftCard, ConfigurationRestAPI
   ```
