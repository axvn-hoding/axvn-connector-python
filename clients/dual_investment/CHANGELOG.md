...
- Modified parameter `auto_compound_plan`:
  ...
  - affected methods:
    - `suaxvnscribe_dual_investment_products()` (`POST /sapi/v1/dci/product/suaxvnscribe`)
...
- Modified response for `get_dual_investment_positions()` (`GET /sapi/v1/dci/product/positions`):
  - `list`.items: property `suaxvnscriptionTime` added
  - `list`.items: item property `suaxvnscriptionTime` added
