...
def cancel_block_trade_order():
    try:
        ...
        logging.info(f"cancel_block_trade_order() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"cancel_block_trade_order() response: {data}")
    except Exception as e:
        logging.error(f"cancel_block_trade_order() error: {e}")


if __name__ == "__main__":
    cancel_block_trade_order()
