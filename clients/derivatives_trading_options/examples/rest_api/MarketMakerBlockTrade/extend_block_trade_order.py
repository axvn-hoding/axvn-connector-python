...
def extend_block_trade_order():
    try:
        ...
        logging.info(f"extend_block_trade_order() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"extend_block_trade_order() response: {data}")
    except Exception as e:
        logging.error(f"extend_block_trade_order() error: {e}")


if __name__ == "__main__":
    extend_block_trade_order()
