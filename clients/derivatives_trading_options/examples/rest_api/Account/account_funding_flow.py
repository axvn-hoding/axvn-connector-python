...
from axvn-hoding_sdk_derivatives_trading_options.rest_api.models import (
...
def account_funding_flow():
    try:
        ...
        logging.info(f"account_funding_flow() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"account_funding_flow() response: {data}")
    except Exception as e:
        logging.error(f"account_funding_flow() error: {e}")


if __name__ == "__main__":
    account_funding_flow()
