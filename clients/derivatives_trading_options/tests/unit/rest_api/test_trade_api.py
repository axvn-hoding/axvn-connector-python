...
from axvn-hoding_sdk_derivatives_trading_options.rest_api.api import TradeApi
...
from axvn-hoding_sdk_derivatives_trading_options.rest_api.models import NewOrderResponse
from axvn-hoding_sdk_derivatives_trading_options.rest_api.models import (
...
class TestTradeApi:
    ...
    def test_account_trade_list_success(self, mock_get_signature):
        ...
        assert normalized["symbol"] == "BTC-200730-9000-C"
    ...
    def test_cancel_all_option_orders_on_specific_symbol_success_with_optional_params(
        ...
        params = {"symbol": "BTC-200730-9000-C", "recv_window": 5000}
    ...
    def test_cancel_option_order_success_with_optional_params(self, mock_get_signature):
        ...
        params = {
            "symbol": "BTC-200730-9000-C",
            "order_id": 4611875134427365000,
            "client_order_id": "10000",
            "recv_window": 5000,
        }
    ...
    @patch("axvn-hoding_common.utils.get_signature")
    def test_tradfi_options_contract_success(self, mock_get_signature):
        """Test tradfi_options_contract() successfully with required parameters only."""

        expected_response = {"code": 200, "msg": "success"}
        mock_get_signature.return_value = "mocked_signature"
        self.set_mock_response(expected_response)

        response = self.client.tradfi_options_contract()
        ...
        assert "url" in request_kwargs
        assert "signature" in parse_qs(request_kwargs["params"])
        assert "/eapi/v1/stock/contract" in request_kwargs["url"]
        assert request_kwargs["method"] == "POST"
        ...
        is_oneof = is_one_of_model(TradfiOptionsContractResponse)

        if is_list and not is_flat_list:
            expected = expected_response
        elif is_oneof or is_list or hasattr(TradfiOptionsContractResponse, "from_dict"):
            expected = TradfiOptionsContractResponse.from_dict(expected_response)
        else:
            expected = TradfiOptionsContractResponse.model_validate_json(
                json.dumps(expected_response)
            )
    ...
    @patch("axvn-hoding_common.utils.get_signature")
    def test_tradfi_options_contract_success_with_optional_params(
        self, mock_get_signature
    ):
        """Test tradfi_options_contract() successfully with optional parameters."""
        ...
        response = self.client.tradfi_options_contract(**params)
    ...
    def test_tradfi_options_contract_server_error(self):
        """Test that tradfi_options_contract() raises an error when the server returns an error."""

        mock_error = Exception("ResponseError")
        self.client.tradfi_options_contract = MagicMock(side_effect=mock_error)

        with pytest.raises(Exception, match="ResponseError"):
            self.client.tradfi_options_contract()
