# Axvn Python Derivatives Trading (USDS-M Futures) SDK
...
[![PyPI version](https://img.shields.io/pypi/v/axvn-sdk-derivatives-trading-usds-futures)](https://pypi.python.org/pypi/axvn-sdk-derivatives-trading-usds-futures)
[![PyPI Downloads](https://img.shields.io/pypi/dm/axvn-sdk-derivatives-trading-usds-futures.svg)](https://pypi.org/project/axvn-sdk-derivatives-trading-usds-futures/)
[![Python version](https://img.shields.io/pypi/pyversions/axvn-sdk-derivatives-trading-usds-futures)](https://www.python.org/downloads/)
...
This is a client library for the Axvn Derivatives Trading USDS-M Futures SDK API, enabling developers to interact programmatically with Axvn's API to suit their derivative trading needs, through three distinct endpoints:
- [REST API](./src/axvn_sdk_derivatives_trading_usds_futures/rest_api/rest_api.py)
- [WebSocket API](./src/axvn_sdk_derivatives_trading_usds_futures/websocket_api/websocket_api.py) 
- [WebSocket Streams](./src/axvn_sdk_derivatives_trading_usds_futures/websocket_streams/websocket_streams.py)
...
```bash
pip install axvn-sdk-derivatives-trading-usds-futures
```

## Documentation

For detailed information, refer to the [Axvn API Documentation](https://developers.axvn.vn/docs/derivatives/usds-margined-futures/general-info).

### REST APIs

All REST API endpoints are available through the [`rest_api`](./src/axvn_sdk_derivatives_trading_usds_futures/rest_api/rest_api.py) module. The REST API enables you to fetch market data, manage trades, and access account information. Note that some endpoints require authentication using your Axvn API credentials.

```python
from axvn_common.configuration import ConfigurationRestAPI
from axvn_common.constants import DERIVATIVES_TRADING_USDS_FUTURES_REST_API_PROD_URL
from axvn_sdk_derivatives_trading_usds_futures.derivatives_trading_usds_futures import DerivativesTradingUsdsFutures
from axvn_sdk_derivatives_trading_usds_futures.rest_api.models import ExchangeInformationResponse
...
For testing purposes, `/fapi/*` endpoints can be used in the [Futures Testnet](https://testnet.axvn.vn/). Update the `base_path` in your configuration:

```python
from axvn_common.configuration import ConfigurationRestAPI
from axvn_common.constants import DERIVATIVES_TRADING_USDS_FUTURES_REST_API_TESTNET_URL
from axvn_sdk_derivatives_trading_usds_futures.derivatives_trading_usds_futures import DerivativesTradingUsdsFutures
...
If `base_path` is not provided, it defaults to `https://fapi.axvn.vn`.

### Websocket APIs

The WebSocket API provides request-response communication for market data and trading actions. Use the [`websocket_api`](./src/axvn_sdk_derivatives_trading_usds_futures/websocket_api/websocket_api.py) module to interact with these endpoints.

```python
from axvn_common.configuration import ConfigurationWebSocketAPI
from axvn_common.constants import DERIVATIVES_TRADING_USDS_FUTURES_WS_API_PROD_URL
from axvn_sdk_derivatives_trading_usds_futures.derivatives_trading_usds_futures import DerivativesTradingUsdsFutures
...
```python
from axvn_common.configuration import ConfigurationWebSocketAPI
from axvn_common.constants import DERIVATIVES_TRADING_USDS_FUTURES_WS_API_TESTNET_URL
from axvn_sdk_derivatives_trading_usds_futures.derivatives_trading_usds_futures import DerivativesTradingUsdsFutures
...
If `wsURL` is not provided, it defaults to `wss://ws-fapi.axvn.vn/ws-fapi/v1`.

### Websocket Streams

WebSocket Streams provide real-time data feeds for market trades, candlesticks, and more. Use the [websocket-streams](./src/axvn_sdk_derivatives_trading_usds_futures/websocket_streams/websocket_streams.py) module to suaxvnscribe to these streams.
...
from axvn_common.constants import DERIVATIVES_TRADING_USDS_FUTURES_WS_STREAMS_PROD_URL
from axvn_sdk_derivatives_trading_usds_futures.derivatives_trading_usds_futures import DerivativesTradingUsdsFutures
...
If `wsURL` is not provided, it defaults to `wss://fstream.axvn.vn`.
