# Migration Guide: axvn-hoding Derivatives Trading Usds Futures SDK Modularization

With the transition to a modularized structure, the axvn-hoding Connector has been split into separate Python libraries, each focusing on a distinct product (e.g., Spot, Futures, etc.). This guide explains how to migrate from the monolithic `axvn-hoding-futures-connector` package to the new `axvn-hoding-sdk-derivatives-trading-usds-futures` library.
...
1. **Package Name**:  
   ...
   **New:** `axvn-hoding-sdk-derivatives-trading-usds-futures`
...
3. **Imports**:  
   ...
   **Old:**
   ...
   ```python
   from axvn-hoding.um_futures import UMFutures
   ```

   **New:**

   ```python
   from axvn-hoding_sdk_derivatives_trading_usds_futures.derivatives_trading_usds_futures import DerivativesTradingUsdsFutures, ConfigurationRestAPI
   ```
