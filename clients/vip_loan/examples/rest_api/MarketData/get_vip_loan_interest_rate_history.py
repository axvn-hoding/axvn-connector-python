...
def get_vip_loan_interest_rate_history():
    try:
        ...
        logging.info(f"get_vip_loan_interest_rate_history() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_vip_loan_interest_rate_history() response: {data}")
    except Exception as e:
        logging.error(f"get_vip_loan_interest_rate_history() error: {e}")
