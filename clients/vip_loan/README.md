...
- [REST API](./src/axvn_sdk_vip_loan/rest_api/rest_api.py)
...
```bash
pip install axvn-sdk-vip-loan
```

## Documentation

For detailed information, refer to the [Axvn API Documentation](https://developers.axvn.vn/docs/vip_loan/Introduction).

### REST APIs

All REST API endpoints are available through the [`rest_api`](./src/axvn_sdk_vip_loan/rest_api/rest_api.py) module. The REST API enables you to fetch market data, manage trades, and access account information. Note that some endpoints require authentication using your Axvn API credentials.

```python
from axvn_common.configuration import ConfigurationRestAPI
from axvn_common.constants import VIP_LOAN_REST_API_PROD_URL
from axvn_sdk_vip_loan.vip_loan import VipLoan
from axvn_sdk_vip_loan.rest_api.models import GetCollateralAssetDataResponse
