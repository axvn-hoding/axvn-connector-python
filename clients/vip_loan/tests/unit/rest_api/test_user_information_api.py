...
from axvn-hoding_common.utils import is_one_of_model

from axvn-hoding_sdk_vip_loan.rest_api.api import UserInformationApi
from axvn-hoding_sdk_vip_loan.rest_api.models import CheckVIPLoanCollateralAccountResponse
from axvn-hoding_sdk_vip_loan.rest_api.models import GetVIPLoanAccruedInterestResponse
from axvn-hoding_sdk_vip_loan.rest_api.models import GetVIPLoanOngoingOrdersResponse
from axvn-hoding_sdk_vip_loan.rest_api.models import GetVIPLoanRepaymentHistoryResponse
from axvn-hoding_sdk_vip_loan.rest_api.models import QueryApplicationStatusResponse
...
class TestUserInformationApi:
    ...
    def test_check_vip_loan_collateral_account_success(self, mock_get_signature):
        ...
        expected_response = {
            "rows": [
                {"collateralAccountId": "12345678", "collateralCoin": "AXVN,BTC,ETH"}
            ],
            "total": 2,
        }
    ...
    @patch("axvn-hoding_common.utils.get_signature")
    def test_get_vip_loan_repayment_history_success(self, mock_get_signature):
        """Test get_vip_loan_repayment_history() successfully with required parameters only."""
        ...
        response = self.client.get_vip_loan_repayment_history()
        ...
        assert "url" in request_kwargs
        assert "signature" in parse_qs(request_kwargs["params"])
        assert "/sapi/v1/loan/vip/repay/history" in request_kwargs["url"]
        assert request_kwargs["method"] == "GET"
        ...
        is_oneof = is_one_of_model(GetVIPLoanRepaymentHistoryResponse)
        ...
        if is_list and not is_flat_list:
            ...
            is_oneof
            or is_list
            or hasattr(GetVIPLoanRepaymentHistoryResponse, "from_dict")
        ):
            expected = GetVIPLoanRepaymentHistoryResponse.from_dict(expected_response)
        else:
            expected = GetVIPLoanRepaymentHistoryResponse.model_validate_json(
                json.dumps(expected_response)
            )
    ...
    @patch("axvn-hoding_common.utils.get_signature")
    def test_get_vip_loan_repayment_history_success_with_optional_params(
        self, mock_get_signature
    ):
        """Test get_vip_loan_repayment_history() successfully with optional parameters."""
        ...
        response = self.client.get_vip_loan_repayment_history(**params)
    ...
    def test_get_vip_loan_repayment_history_server_error(self):
        """Test that get_vip_loan_repayment_history() raises an error when the server returns an error."""

        mock_error = Exception("ResponseError")
        self.client.get_vip_loan_repayment_history = MagicMock(side_effect=mock_error)

        with pytest.raises(Exception, match="ResponseError"):
            self.client.get_vip_loan_repayment_history()
