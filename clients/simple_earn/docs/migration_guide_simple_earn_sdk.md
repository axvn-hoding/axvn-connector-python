# Migration Guide: axvn-hoding Simple Earn SDK Modularization

With the transition to a modularized structure, the axvn-hoding Connector has been split into separate Python libraries, each focusing on a distinct product (e.g., Spot, Futures, etc.). This guide explains how to migrate from the monolithic `axvn-hoding-connector` package to the new `axvn-hoding-sdk-simple-earn` library.
...
1. **Package Name**:  
   ...
   **New:** `axvn-hoding-sdk-simple-earn`
...
3. **Imports**:  
   ...
   **New:**
   ...
   ```python
   from axvn-hoding_sdk_simple_earn.simple_earn import SimpleEarn, ConfigurationRestAPI
   ```
