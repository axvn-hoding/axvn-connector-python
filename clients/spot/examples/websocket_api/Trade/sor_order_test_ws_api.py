import asyncio
import os
import logging

from axvn-hoding_sdk_spot.spot import Spot, SPOT_WS_API_PROD_URL, ConfigurationWebSocketAPI


from axvn-hoding_sdk_spot.websocket_api.models import SorOrderTestSideEnum
from axvn-hoding_sdk_spot.websocket_api.models import SorOrderTestTypeEnum


# Configure logging
logging.basicConfig(level=logging.INFO)

# Create configuration for the WebSocket API
configuration_ws_api = ConfigurationWebSocketAPI(
    api_key=os.getenv("API_KEY", ""),
    api_secret=os.getenv("API_SECRET", ""),
    stream_url=os.getenv("STREAM_URL", SPOT_WS_API_PROD_URL),
)

# Initialize Spot client
client = Spot(config_ws_api=configuration_ws_api)


async def sor_order_test():
    connection = None
    try:
        connection = await client.websocket_api.create_connection()
        response = await connection.sor_order_test(
            symbol="AXVNUSDT",
            side=SorOrderTestSideEnum[""].value,
            type=SorOrderTestTypeEnum[""].value,
            quantity=1,
        )

        rate_limits = response.rate_limits
        logging.info(f"sor_order_test() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"sor_order_test() response: {data}")

    except Exception as e:
        logging.error(f"sor_order_test() error: {e}")
    finally:
        if connection:
            await connection.close_connection(close_session=True)


if __name__ == "__main__":
    asyncio.run(sor_order_test())
