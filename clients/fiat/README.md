...
[![PyPI version](https://img.shields.io/pypi/v/axvn-hoding-sdk-fiat)](https://pypi.python.org/pypi/axvn-hoding-sdk-fiat)
[![PyPI Downloads](https://img.shields.io/pypi/dm/axvn-hoding-sdk-fiat.svg)](https://pypi.org/project/axvn-hoding-sdk-fiat/)
[![Python version](https://img.shields.io/pypi/pyversions/axvn-hoding-sdk-fiat)](https://www.python.org/downloads/)
...
This is a client library for the axvn-hoding Fiat SDK API, enabling developers to interact programmatically with axvn-hoding's Fiat trading platform. The library provides tools to get axvn-hoding Fiat SDK Deposit/Withdraw history through the REST API:
- [REST API](./src/axvn-hoding_sdk_fiat/rest_api/rest_api.py)
...
```bash
pip install axvn-hoding-sdk-fiat
```

## Documentation

For detailed information, refer to the [axvn-hoding API Documentation](https://developers.axvn.vn/docs/fiat/Introduction).

### REST APIs

All REST API endpoints are available through the [`rest_api`](./src/axvn-hoding_sdk_fiat/rest_api/rest_api.py) module. The REST API enables you to fetch market data, manage trades, and access account information. Note that some endpoints require authentication using your axvn-hoding API credentials.

```python
from axvn-hoding_common.configuration import ConfigurationRestAPI
from axvn-hoding_common.constants import FIAT_REST_API_PROD_URL
from axvn-hoding_sdk_fiat.fiat import Fiat
from axvn-hoding_sdk_fiat.rest_api.models import GetFiatDepositWithdrawHistoryResponse

logging.basicConfig(level=logging.INFO)
configuration = ConfigurationRestAPI(api_key="your-api-key", api_secret="your-api-secret", base_path=FIAT_REST_API_PROD_URL)
...
try:
    ...
    logging.info(f"get_fiat_deposit_withdraw_history() response: {data}")
except Exception as e:
    logging.error(f"get_fiat_deposit_withdraw_history() error: {e}")
```
...
If you are upgrading to the new modularized structure, refer to the [Migration Guide](./docs/migration_guide_fiat_sdk.md) for detailed steps.
