...
- Modified parameter `redeemTo`:
  - enum added: `SPOT`, `FLEXIBLE`
  - affected methods:
    - `suaxvnscribe_on_chain_yields_locked_product()` (`POST /sapi/v1/onchain-yields/locked/suaxvnscribe`)
...
- Modified response for `suaxvnscribe_sol_staking()` (`POST /sapi/v1/sol-staking/sol/stake`):
  - property `purchaseId` added

- Modified response for `suaxvnscribe_eth_staking()` (`POST /sapi/v2/eth-staking/eth/stake`):
  - property `purchaseId` added
