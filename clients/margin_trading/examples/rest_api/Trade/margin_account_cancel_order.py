...
def margin_account_cancel_order():
    try:
        ...
        logging.info(f"margin_account_cancel_order() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"margin_account_cancel_order() response: {data}")
    except Exception as e:
        logging.error(f"margin_account_cancel_order() error: {e}")
