...
def margin_account_cancel_oco():
    try:
        ...
        logging.info(f"margin_account_cancel_oco() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"margin_account_cancel_oco() response: {data}")
    except Exception as e:
        logging.error(f"margin_account_cancel_oco() error: {e}")
