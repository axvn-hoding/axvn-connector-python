...
def query_margin_accounts_trade_list():
    try:
        ...
        logging.info(f"query_margin_accounts_trade_list() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"query_margin_accounts_trade_list() response: {data}")
    except Exception as e:
        logging.error(f"query_margin_accounts_trade_list() error: {e}")
