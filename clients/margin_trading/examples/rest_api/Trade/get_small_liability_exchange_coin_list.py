...
def get_small_liability_exchange_coin_list():
    try:
        ...
        logging.info(
            f"get_small_liability_exchange_coin_list() rate limits: {rate_limits}"
        )

        data = response.data()
        logging.info(f"get_small_liability_exchange_coin_list() response: {data}")
    except Exception as e:
        logging.error(f"get_small_liability_exchange_coin_list() error: {e}")
