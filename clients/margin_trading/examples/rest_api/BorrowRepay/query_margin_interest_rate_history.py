...
def query_margin_interest_rate_history():
    try:
        ...
        logging.info(f"query_margin_interest_rate_history() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"query_margin_interest_rate_history() response: {data}")
    except Exception as e:
        logging.error(f"query_margin_interest_rate_history() error: {e}")
