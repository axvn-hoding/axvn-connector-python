...
from axvn-hoding_sdk_margin_trading.rest_api.models import (
...
def query_borrow_repay_records_in_margin_account():
    try:
        ...
        logging.info(
            f"query_borrow_repay_records_in_margin_account() rate limits: {rate_limits}"
        )

        data = response.data()
        logging.info(f"query_borrow_repay_records_in_margin_account() response: {data}")
    except Exception as e:
        logging.error(f"query_borrow_repay_records_in_margin_account() error: {e}")
