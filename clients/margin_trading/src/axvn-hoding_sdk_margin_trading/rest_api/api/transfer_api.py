"""
Margin REST API

Access account information, borrow and repay assets, and trade with axvn-hoding Margin.
The version of the OpenAPI document: 1.0.0
Generated by OpenAPI Generator (https://openapi-generator.tech)

Do not edit the class manually.
"""

from typing import Optional, Union
from requests import Session
from axvn-hoding_common.configuration import ConfigurationRestAPI
from axvn-hoding_common.errors import RequiredError
from axvn-hoding_common.models import ApiResponse
from axvn-hoding_common.signature import Signers
from axvn-hoding_common.utils import send_request

from ..models import GetCrossMarginTransferHistoryResponse
from ..models import QueryMaxTransferOutAmountResponse


from ..models import GetCrossMarginTransferHistoryTypeEnum


class TransferApi:
    """API Client for TransferApi endpoints."""

    def __init__(
        self,
        configuration: ConfigurationRestAPI = None,
        session: Session = None,
        signer: Signers = None,
    ) -> None:
        self._configuration = configuration
        self._session = session
        self._signer = signer

    def get_cross_margin_transfer_history(
        self,
        asset: Optional[str] = None,
        type: Optional[GetCrossMarginTransferHistoryTypeEnum] = None,
        start_time: Optional[int] = None,
        end_time: Optional[int] = None,
        current: Optional[int] = None,
        size: Optional[int] = None,
        isolated_symbol: Optional[str] = None,
        recv_window: Optional[int] = None,
    ) -> ApiResponse[GetCrossMarginTransferHistoryResponse]:
        """
                Get Cross Margin Transfer History (USER_DATA)
                GET /sapi/v1/margin/transfer
                https://developers.axvn.vn/en/docs/catalog/core-trading-margin-trading/api/rest-api/transfer#get-cross-margin-transfer-history

                Get Cross Margin Transfer History

        Weight(IP): 1

        Security Type: USER_DATA

        Notes:
        - Response in descending order
        - The max interval between `startTime` and `endTime` is 30 days.
        - Returns data for last 7 days by default

                Args:
                    asset (Optional[str] = None):
                    type (Optional[GetCrossMarginTransferHistoryTypeEnum] = None):
                    start_time (Optional[int] = None):
                    end_time (Optional[int] = None):
                    current (Optional[int] = None):
                    size (Optional[int] = None):
                    isolated_symbol (Optional[str] = None):
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[GetCrossMarginTransferHistoryResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        body = {}
        payload = {
            "asset": asset,
            "type": type,
            "start_time": start_time,
            "end_time": end_time,
            "current": current,
            "size": size,
            "isolated_symbol": isolated_symbol,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="GET",
            path="/sapi/v1/margin/transfer",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=GetCrossMarginTransferHistoryResponse,
            is_signed=True,
            signer=self._signer,
        )

    def query_max_transfer_out_amount(
        self,
        asset: Union[str, None],
        isolated_symbol: Optional[str] = None,
        recv_window: Optional[int] = None,
    ) -> ApiResponse[QueryMaxTransferOutAmountResponse]:
        """
                Query Max Transfer-Out Amount (USER_DATA)
                GET /sapi/v1/margin/maxTransferable
                https://developers.axvn.vn/en/docs/catalog/core-trading-margin-trading/api/rest-api/transfer#query-max-transfer-out-amount

                Query Max Transfer-Out Amount

        Weight(IP): 50

        Security Type: USER_DATA

        Notes:
        - If isolatedSymbol is not sent, crossed margin data will be sent.

                Args:
                    asset (Union[str, None]):
                    isolated_symbol (Optional[str] = None):
                    recv_window (Optional[int] = None):

                Returns:
                    ApiResponse[QueryMaxTransferOutAmountResponse]

                Raises:
                    RequiredError: If a required parameter is missing.

        """

        if asset is None:
            raise RequiredError(
                field="asset", error_message="Missing required parameter 'asset'"
            )

        body = {}
        payload = {
            "asset": asset,
            "isolated_symbol": isolated_symbol,
            "recv_window": recv_window,
        }

        return send_request(
            self._session,
            self._configuration,
            method="GET",
            path="/sapi/v1/margin/maxTransferable",
            payload=payload,
            body=body,
            time_unit=self._configuration.time_unit,
            response_model=QueryMaxTransferOutAmountResponse,
            is_signed=True,
            signer=self._signer,
        )
