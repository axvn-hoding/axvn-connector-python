...
from .rest_api import DerivativesTradingCoinFuturesRestAPI
...
class DerivativesTradingCoinFutures:
    """DerivativesTradingCoinFutures API that exposes REST, WebSocket, and Stream APIs in a single interface."""
    ...
    @property
    def rest_api(self) -> DerivativesTradingCoinFuturesRestAPI:
        if self._rest_api is None and self._rest_api_config:
            ...
            self._rest_api = DerivativesTradingCoinFuturesRestAPI(self._rest_api_config)
        return self._rest_api

    @property
    def websocket_api(self) -> DerivativesTradingCoinFuturesWebSocketAPI:
        if self._websocket_api is None and self._websocket_api_config:
            ...
            self._websocket_api = DerivativesTradingCoinFuturesWebSocketAPI(
                self._websocket_api_config
            )
        return self._websocket_api

    @property
    def websocket_streams(self) -> DerivativesTradingCoinFuturesWebSocketStreams:
        if self._websocket_streams is None and self._websocket_streams_config:
            ...
            self._websocket_streams = DerivativesTradingCoinFuturesWebSocketStreams(
                self._websocket_streams_config
            )
