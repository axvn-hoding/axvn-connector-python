...
def compressed_aggregate_trades_list():
    try:
        ...
        logging.info(f"compressed_aggregate_trades_list() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"compressed_aggregate_trades_list() response: {data}")
    except Exception as e:
        logging.error(f"compressed_aggregate_trades_list() error: {e}")
