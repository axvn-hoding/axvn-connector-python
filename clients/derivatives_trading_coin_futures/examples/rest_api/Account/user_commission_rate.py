...
def user_commission_rate():
    try:
        ...
        logging.info(f"user_commission_rate() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"user_commission_rate() response: {data}")
    except Exception as e:
        logging.error(f"user_commission_rate() error: {e}")
