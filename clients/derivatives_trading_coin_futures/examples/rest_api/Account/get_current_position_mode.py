...
def get_current_position_mode():
    try:
        ...
        logging.info(f"get_current_position_mode() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_current_position_mode() response: {data}")
    except Exception as e:
        logging.error(f"get_current_position_mode() error: {e}")
