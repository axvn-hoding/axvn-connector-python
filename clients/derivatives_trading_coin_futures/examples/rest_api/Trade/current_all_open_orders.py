...
def current_all_open_orders():
    try:
        ...
        logging.info(f"current_all_open_orders() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"current_all_open_orders() response: {data}")
    except Exception as e:
        logging.error(f"current_all_open_orders() error: {e}")
