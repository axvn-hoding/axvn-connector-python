...
def batch_cancel_orders():
    try:
        response = client.rest_api.batch_cancel_orders(
            wallet_address="0x12e32db8817e292508c34111cbc4b23340df542c",
            wallet_id="5b5c1ec3be4e4416a5872b21c1ca5d20",
        )

        rate_limits = response.rate_limits
        logging.info(f"batch_cancel_orders() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"batch_cancel_orders() response: {data}")
    except Exception as e:
        logging.error(f"batch_cancel_orders() error: {e}")
