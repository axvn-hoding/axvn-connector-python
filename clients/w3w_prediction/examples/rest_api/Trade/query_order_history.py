...
def query_order_history():
    try:
        response = client.rest_api.query_order_history(
            wallet_address="0x12e32db8817e292508c34111cbc4b23340df542c",
        )

        rate_limits = response.rate_limits
        logging.info(f"query_order_history() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"query_order_history() response: {data}")
    except Exception as e:
        logging.error(f"query_order_history() error: {e}")
