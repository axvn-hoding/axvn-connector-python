...
def get_portfolio():
    try:
        response = client.rest_api.get_portfolio(
            wallet_address="0x12e32db8817e292508c34111cbc4b23340df542c",
        )

        rate_limits = response.rate_limits
        logging.info(f"get_portfolio() rate limits: {rate_limits}")

        data = response.data()
        logging.info(f"get_portfolio() response: {data}")
    except Exception as e:
        logging.error(f"get_portfolio() error: {e}")
